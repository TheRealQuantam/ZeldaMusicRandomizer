...
"""
Basic usage:
    1. Select the ROM to use as a base, either the original Legend of Zelda ROM or a ROM generated by the Legend of Zelda randomizer.
    2. Select whether to include original tracks and the tracks from the standard library.
    3. Select whether to exclude unsafe tracks that may interfere with streaming.
    4. (OPTIONAL) Select the folder containing any custom music libraries in the appropriate JSON5 format, if any.
    5. (OPTIONAL) To reproduce the music selections from a previous output ROM, enter the seed number desired.
    6. Click Randomize.
...
""";
    ...
    public async void SelectRomPath_Clicked()
    {
        ...
        if (isVanilla)
        {
            ...
            romTypeStr = "Non-vanilla";

        AppendLogLine($"\n🛈 {romTypeStr} ROM selected");
    ...
    public async void Randomize_Clicked()
    {
        try
        {
            ...
            if (songs.Count == 0)
            {
                AppendLogLine("\n❌ No songs were selected and no ROM was generated");
                return;
            }

            // Get output path
            string flagsStr = (IsRandomizerRom ? "R" : "")
                + (IncludeOriginalTracks ? "O" : "")
                + (IncludeStandardLibraryTracks ? "S" : "")
                + (ExcludeUnsafeTracks ? "" : "U");
            string defFilename = $"{Path.GetFileNameWithoutExtension(_romFile.Name)} Music {flagsStr} {seed}{Path.GetExtension(_romFile.Name)}";
    ...
    void AppendLogLine(string s, bool scrollToEnd = true)
    {
        Log += $"{s}\n";
